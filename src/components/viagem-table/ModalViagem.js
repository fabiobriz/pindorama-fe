...
import { alterarViagem, excluirViagem, obterViagem } from "../../net/service"
...
function ModalViagem({ idCliente, title }) {
    ...
    useEffect(() => {
        ...
        if ((idCliente !== 0) && (idCliente !== null)) {
            obterViagem(idCliente).then((result) => {
    ...
    }, [idCliente, oldCliente.cpf, oldCliente.data_ida, oldCliente.data_volta, oldCliente.destino, oldCliente.idade, oldCliente.nome, oldCliente.origem, oldCliente.valor, oldCliente.viagem.data_ida, oldCliente.viagem.data_volta, oldCliente.viagem.destino, oldCliente.viagem.id, oldCliente.viagem.origem, oldCliente.viagem.valor])



    const handleClick = (e) => {
        e.preventDefault()
        const newCliente = { nome, idade, cpf,  idViagem, origem, destino, valor, data_ida, data_volta}

        if (title === "Alterar") {
            alterarViagem(idCliente, newCliente).then(window.location.reload());
        } else if (title === "Apagar") {
            excluirViagem(idCliente).then(window.location.reload());
        }
    }
